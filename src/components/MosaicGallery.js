        ...
        <div ref={props.animRef} id={props.id} className="mosaic-gallery">
            ...
            {[...props.images].map((i,index)=>(
                <div key={"mosaicImage"+index} className={"mosaic-image-container fade-int " + props.id}>
                    <img className="mosaic-image" src={i} title={(props.titles===undefined)?("mosaicImage"+(index+1)):(props.titles[index])} alt={(props.titles===undefined)?("mosaicImage"+(index+1)):(props.titles[index])} onClick={()=>window.open(i,'_blank')} />
                </div>
            ))}
